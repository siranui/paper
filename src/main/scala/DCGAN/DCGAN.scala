...
object atMNIST {
  ...
  val train_d = utils.read("/home/share/number/train-d.txt")
  val test_d = utils.read("/home/share/number/test-d.txt")

  val D = Discriminator() // Loss-func is 'cross entropy'
  val G = Generator() // Loss-func is 'cross entropy'
  val dcgan = DCGAN(G, D)

  def train(BATCH_SIZE: Int, NUM_EPOCH: Int, GENERATED_IMAGE_PATH: String) = {
    println("train start")
  ...
  def main(args: Array[String]) {
    ...
    train(BATCH_SIZE, NUM_EPOCH, GENERATED_IMAGE_PATH)
