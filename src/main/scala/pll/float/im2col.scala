...
object Im2Col {
  ...
  def im2col(x: Array[DV], fil_h: Int, fil_w: Int, ch: Int = 1, stride: Int = 1) = {
    ...
    val col = DenseMatrix.zeros[T](ch * fil_h * fil_w, out_w * out_w * images.size)
  ...
  def col2im(col: DM, x_shape: Shape, fil_h: Int, fil_w: Int, stride: Int = 1): Array[Array[DM]] = {
    ...
    for {
      ...
      im(b)(c)(oh * stride until oh * stride + fil_h, ow * stride until ow * stride + fil_w) += reshape(
...
/**
  ...
  */
case class i2cConv(input_width: Int,
                   filter_width: Int,
                   filter_set: Int = 1,
                   channel: Int = 1,
                   stride: Int = 1,
  ...
  def backward(d: DV): DV = {
    ...
    val dxVec = dx
      .map(c => c.map(_.t.toDenseVector).reduce(DenseVector.vertcat(_, _)))
  ...
  override def load(data: List[String]): List[String] = {
    ...
    pll.log.debug(s"\tB: ${B.size}, loaded: ${blst.size}")
    ...
    val dd = opt_filter.load(data.drop(2))
    opt_bias.load(dd)
  }
  ...
  override def duplicate(): i2cConv = {
    val dup =
      i2cConv(input_width, filter_width, filter_set, channel, stride, distr, SD, update_method, lr)
    dup.F = this.copy_F()
    dup.B = this.copy_B()
    dup
  }
