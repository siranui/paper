...
case class k2rConv(input_width: Int,
  ...
  var xs: Option[List[ADV]]  = None //storage: input by channel
  var Ws: Option[Array[ADM]] = None
  ...
  var B: ADV = Array
    .ofDim[DV](filter_set)
    .map(_ => DenseVector.zeros[T](out_width * out_width))

  var dF: Array[ADV] = F.map(_.map(_ => DenseVector.zeros[T](filter_width * filter_width)))
  var dB: ADV        = B.map(_ => DenseVector.zeros[T](out_width * out_width))
  ...
  def forward(x: DV): DV = {
    val xs: ADV = utils.divideIntoN(x, N = channel)
    ...
    val buf = DenseMatrix.zeros[T](filter_set, input_width * input_width)
  ...
  def backward(d: DV): DV = {
    ...
    val ds: ADV = utils.divideIntoN(d, N = filter_set)
    val dmat = ds
      .map { m =>
    ...
    val dx = DenseMatrix.zeros[T](channel, input_width * input_width)
    ((0 until filter_width * filter_width) zip (for (i <- 0 until input_width * input_width
                                                     if (i % input_width < filter_width)) yield i))
      .foreach {
        case (idx, num) =>
  ...
  def reset(): Unit = {
    xs = None
    Ws = None
    dF = dF.map(_.map(_ => DenseVector.zeros[T](filter_width * filter_width)))
    dB = dB.map(_ => DenseVector.zeros[T](out_width * out_width))
  }
  ...
  def copy_B(): ADV = B.map(_.copy)

  override def duplicate(): k2rConv = {
    val dup =
      k2rConv(input_width, filter_width, filter_set, channel, stride, distr, SD, update_method, lr)
    dup.F = this.copy_F()
    dup.B = this.copy_B()
    dup
  }
...
object k2rConvTest {
  ...
  def o(n: Int): DenseVector[T] = DenseVector.ones[T](n)
  def r(n: Int): DenseVector[T] = DenseVector.range(0, n).map(v => (v: T))
  val in_w: Int                 = 32
  val fil_w: Int                = 7
  val stride: Int               = 1
  val out_w_s: Int              = utils.out_width(in_w, fil_w, stride)
  val out_w: Int                = in_w - fil_w + 1
  val f_set: Int                = 8
  val ch: Int                   = 6
  val (f, x, d, ds) = (
    r(fil_w * fil_w),
