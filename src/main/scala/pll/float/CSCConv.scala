...
case class CSCConv(
  ...
  var xs: Option[List[ADV]]                  = None //チャネルごとの入力を格納
  var Ws: Option[Array[Array[CSCMatrix[T]]]] = None
  ...
  def forward(x: DV): DV = {
    ...
    val Ws = Array.ofDim[CSCMatrix[T]](filter_set, channel)
  ...
  def backward(d: DV): DV = {
    ...
    val dWs: Array[ADM] = dmap.map(d => xs.map(x => d * x.t))
    ...
    val Ws = this.Ws.get
    val dx: DV = (for { fs <- 0 until filter_set } yield {
      (for { ch <- 0 until channel } yield {
        val tmp: DV = (dmap(fs).t * Ws(fs)(ch).toDense).t
  ...
  // 入力画像とフィルターは正方形に限定
  def filter2Weight(filter: DV, input_size: Int, stride: Int): CSCMatrix[T] = {
    val in_w   = math.sqrt(input_size).toInt
    val h: Int = math.sqrt(filter.length).toInt
    val out_w  = pll.utils.out_width(in_w, h, stride)

    val W =
      new CSCMatrix.Builder[T](rows = math.pow(out_w, 2).toInt, cols = math.pow(in_w, 2).toInt)
  ...
  override def duplicate(): CSCConv = {
    val dup =
      CSCConv(input_width, filter_width, filter_set, channel, stride, distr, SD, update_method, lr)
    dup.F = this.copy_F()
    dup.B = this.copy_B()
    dup
  }
