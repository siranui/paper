...
trait Opt extends Load {
  def update(ps: Array[DenseMatrix[T]], ds: Array[DenseMatrix[T]]): Array[DenseMatrix[T]];
  def update(ps: Array[DenseVector[T]], ds: Array[DenseVector[T]]): Array[DenseVector[T]];
  def register(ps: Array[DenseMatrix[T]]);
  def register(ps: Array[DenseVector[T]]);
  def save(fn: String);
...
class SGD(var lr: T = 0.01) extends Opt {
  def register(ps: Array[DenseMatrix[T]]) {}
  def register(ps: Array[DenseVector[T]]) {}

  def update(ps: Array[DenseMatrix[T]], ds: Array[DenseMatrix[T]]): Array[DenseMatrix[T]] = {
    val us = new Array[DenseMatrix[T]](ps.size)
  ...
  def update(ps: Array[DenseVector[T]], ds: Array[DenseVector[T]]): Array[DenseVector[T]] = {
    val us = new Array[DenseVector[T]](ps.size)
    for (i <- 0 until ps.size) {
      us(i) = lr * ds(i)
    }
    us
...
class Momentum(var lr: T = 0.01, var momentum: T = 0.9) extends Opt {
  var vms = Array[DenseMatrix[T]]()
  var vvs = Array[DenseVector[T]]()

  def register(ps: Array[DenseMatrix[T]]) {
    vms = new Array[DenseMatrix[T]](ps.size)
    for (i <- 0 until vms.size)
      vms(i) = DenseMatrix.zeros[T](ps(i).rows, ps(i).cols)
  }

  def register(ps: Array[DenseVector[T]]) {
    vvs = new Array[DenseVector[T]](ps.size)
    for (i <- 0 until vvs.size)
      vvs(i) = DenseVector.zeros[T](ps(i).size)
  }
...
class AdaGrad(var lr: T = 0.01, var epsilon: T = 1e-8) extends Opt {
  var hms = Array[DenseMatrix[T]]()
  var hvs = Array[DenseVector[T]]()

  def register(ps: Array[DenseMatrix[T]]) {
    hms = new Array[DenseMatrix[T]](ps.size)
    for (i <- 0 until hms.size)
      hms(i) = DenseMatrix.zeros[T](ps(i).rows, ps(i).cols)
  }

  def register(ps: Array[DenseVector[T]]) {
    hvs = new Array[DenseVector[T]](ps.size)
    for (i <- 0 until hvs.size)
      hvs(i) = DenseVector.zeros[T](ps(i).size)
  }
  ...
  def update(ps: Array[DenseMatrix[T]], ds: Array[DenseMatrix[T]]): Array[DenseMatrix[T]] = {
    ...
    for (i <- 0 until ps.size) {
      ...
      us(i) = lr * ds(i) /:/ (hms(i).map(m => (Math.sqrt(m): T)) + epsilon)
    }
    us
  }
  ...
  def update(ps: Array[DenseVector[T]], ds: Array[DenseVector[T]]): Array[DenseVector[T]] = {
    ...
    for (i <- 0 until ps.size) {
      ...
      us(i) = lr * ds(i) /:/ (hvs(i).map(v => (Math.sqrt(v): T)) + epsilon)
    }
    us
  }
  def save(fn: String) {}
...
class Adam(val lr: T = 0.001, val beta1: T = 0.9, val beta2: T = 0.999, val epsilon: T = 1e-8)
    extends Opt {
  var vms = Array[DenseMatrix[T]]()
  var hms = Array[DenseMatrix[T]]()
  var vvs = Array[DenseVector[T]]()
  ...
  def update(ps: Array[DenseMatrix[T]], ds: Array[DenseMatrix[T]]): Array[DenseMatrix[T]] = {
    ...
    val us = new Array[DenseMatrix[T]](ps.size)
    for (i <- 0 until ps.size) {
      vms(i) = beta1 * vms(i) + ((1: T) - beta1) * ds(i)
      hms(i) = beta2 * hms(i) + ((1: T) - beta2) * (ds(i) *:* ds(i))
      val vvms: DenseMatrix[T] = vms(i) / ((1d - Math.pow(beta1, t)): T)
      val hhms: DenseMatrix[T] = hms(i) / ((1d - Math.pow(beta2, t)): T)
      us(i) = lr * vvms /:/ (hhms.map(m => (Math.sqrt(m): T)) + epsilon)
    }
    us
  }
  ...
  def update(ps: Array[DenseVector[T]], ds: Array[DenseVector[T]]): Array[DenseVector[T]] = {
    ...
    val us = new Array[DenseVector[T]](ps.size)
    for (i <- 0 until ps.size) {
      vvs(i) = beta1 * vvs(i) + ((1: T) - beta1) * ds(i)
      hvs(i) = beta2 * hvs(i) + ((1: T) - beta2) * (ds(i) *:* ds(i))
      val vvvs: DenseVector[T] = vvs(i) / ((1d - Math.pow(beta1, t)): T)
      val hhvs: DenseVector[T] = hvs(i) / ((1d - Math.pow(beta2, t)): T)
      us(i) = lr * vvvs /:/ (hhvs.map(v => (Math.sqrt(v): T)) + epsilon)
    }
    us
  }
  def save(fn: String) {
...
class RMSProp(val lr: T = 0.001, val beta: T = 0.9, val epsilon: T = 1e-8) extends Opt {
  var hms = Array[DenseMatrix[T]]()
  var hvs = Array[DenseVector[T]]()
  ...
  def update(ps: Array[DenseMatrix[T]], ds: Array[DenseMatrix[T]]): Array[DenseMatrix[T]] = {
    val us = new Array[DenseMatrix[T]](ps.size)
    for (i <- 0 until ps.size) {
      hms(i) = beta * hms(i) + ((1: T) - beta) * (ds(i) *:* ds(i))
      us(i) = lr * ds(i) /:/ (hms(i).map(m => (Math.sqrt(m): T)) + epsilon)
    }
    us
  }
  ...
  def update(ps: Array[DenseVector[T]], ds: Array[DenseVector[T]]): Array[DenseVector[T]] = {
    val us = new Array[DenseVector[T]](ps.size)
    for (i <- 0 until ps.size) {
      hvs(i) = beta * hvs(i) + ((1: T) - beta) * (ds(i) *:* ds(i))
      us(i) = lr * ds(i) /:/ (hvs(i).map(v => (Math.sqrt(v): T)) + epsilon)
    }
    us
  }
