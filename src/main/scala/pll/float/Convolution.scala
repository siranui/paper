...
case class Convolution(
  ...
  def forward(x: DV): DV = {
    ...
    val Ws = Array.ofDim[DM](filter_set, channel)
  ...
  def backward(d: DV): DV = {
    ...
    val dx: DV = (for { fs <- 0 until filter_set } yield {
      (for { ch <- 0 until channel } yield {
        val tmp: DV = (dmap(fs).t * Ws(fs)(ch)).t
  ...
  def filter2Weight(filter: DV, input_size: Int, stride: Int): DM = {
    ...
    val W = DenseMatrix.zeros[T](math.pow(out_w, 2).toInt, math.pow(w, 2).toInt)
    for {
      i <- 0 until out_w
      j <- 0 until out_w
      p <- 0 until h
