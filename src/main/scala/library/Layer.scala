...
trait Layer {
  ...
  def forwards(x: Array[DenseVector[Double]]): Array[DenseVector[Double]] = {x.map(forward(_))}
  def backward(d: DenseVector[Double]): DenseVector[Double]
  def backwards(d: Array[DenseVector[Double]]): Array[DenseVector[Double]] = {d.map(backward(_))}
  def update() : Unit
  def reset() : Unit
