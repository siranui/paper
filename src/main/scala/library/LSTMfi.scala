package pll
class LSTMfi(val xn:Int,val hn:Int,val dist:String,val n:Double,val u:String,val a:Double) extends Layer{
  ...
  def update(){
    ...
    val dwxh = opt.update(Array(wfx,wcx,wox,wfh,wch,woh),Array(dwfx,dwcx,dwox,dwfh,dwch,dwoh))
    wfx -= dwxh(0)
    wcx -= dwxh(1)
    wox -= dwxh(2)
    wfh -= dwxh(3)
