...
case class psConv(
  ...
  var xs: Option[Array[DenseVector[Double]]] = None //チャネルごとにパディングした後の入力を格納
  var Ws: Option[Array[Array[DenseMatrix[Double]]]] = None

  val out_width = (math.floor((input_width + pad * 2 - filter_width) / stride) + 1).toInt
  ...
  def forward(x: DenseVector[Double]): DenseVector[Double] = {
    val xmap: Array[DenseVector[Double]] = divideIntoN(x, N = channel)
    val xs: Array[DenseVector[Double]] = pad match {
      case 0 => xmap
      case _ => xmap.map(padding(_, mode = 0d, pad))
    }
    this.xs = Some(xs) //パディングした後の入力を保持
    ...
    val u = for (fs <- 0 until filter_set) yield {
      val m = for (ch <- 0 until channel) yield {
        Ws(fs)(ch) = filter2Weight(F(fs)(ch), input_width * input_width, pad, stride)
        Ws(fs)(ch) * xs(ch) // = W * x
      }
  ...
  def backward(d: DenseVector[Double]): DenseVector[Double] = {
    ...
    for{ i <- dWs.indices; j <- dWs(i).indices }{
      dF(i)(j) += Weight2filter(dWs(i)(j), filter_width*filter_width, pad, stride)
    }
    ...
    val dx: DenseVector[Double] = (for { fs <- 0 until filter_set } yield {
      (for { ch <- 0 until channel } yield {
        ...
        val tmp3 = tmp2(pad until wid - pad , pad until wid - pad)
...
// {{{ helper
  ...
  // 入力画像とフィルターは正方形に限定
  def filter2Weight(filter: DenseVector[Double], input_size: Int, pad: Int, stride: Int): DenseMatrix[Double] = {
    val w = math.sqrt(input_size).toInt
    val h = math.sqrt(filter.length).toInt
    val out_w: Int = (math.floor((w + pad * 2 - h) / stride) + 1).toInt
    // println(out_w)

    val W = DenseMatrix.zeros[Double](math.pow(out_w, 2).toInt, math.pow(w + pad * 2, 2).toInt)
   ...
   */
  def filter2Weight(filters: Array[DenseVector[Double]], input_size: Int, pad: Int = 0, stride: Int = 1): DenseMatrix[Double] = {
    val Ws = for(i <- filters) yield {
      filter2Weight(i, input_size, pad, stride)
    }
    Ws.reduceLeft(DenseMatrix.horzcat(_,_))
  }

  def Weight2filter(dmat: DenseMatrix[Double], filter_size: Int, pad: Int = 0, stride: Int = 1): DenseVector[Double] = {
   ...
   *  パディングされた行列
   */
  def padding ( m: DenseMatrix[Double], mode: Double, width: Int ): DenseMatrix[Double] = {
    val pl = padLeft(m, (m.rows+width,m.cols+width), mode)
    padRight(pl, (pl.rows+width, pl.cols+width), mode)
  }

  def padding ( m: DenseVector[Double], mode: Double, width: Int ): DenseVector[Double] = {
    val w = math.sqrt(m.size).toInt
    val mat = reshape(m, w, w)
    padding(mat.t,mode,width).t.toDenseVector
  }
  ...
  override def duplicate()={
    val dup = new psConv( input_width, filter_width, filter_set, channel, pad, stride, distr, SD, update_method, lr )
    dup.F = this.copy_F
    dup.B = this.copy_B
    dup
  }
...
object psConvTest {
  def main(args: Array[String]) {
    ...
    //    channel: Int = 1, pad: Int = 0, stride: Int = 1 )
    val fH = 3
    val fS = 2
    val ch = 3
    val pad = fH / 2 // same size
    val stride = 2
    val input_width = 10
    ...
    val lr: Double = 0.01
    val conv = new psConv(input_width, fH, fS, ch, pad, stride, distr, SD, update_method, lr)
