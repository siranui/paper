...
// {{{ helper
  ...
  override def duplicate()={
    val dup = new sConv( input_width, filter_width, filter_set, channel, stride, distr, SD, update_method, lr )
    dup.F = this.copy_F
    dup.B = this.copy_B
    dup
  }
...
object sConvTest {
  def main(args: Array[String]) {
    ...
    val lr: Double = 0.01
    val conv = new sConv(input_width, fH, fS, ch, stride, distr, SD, update_method, lr)
