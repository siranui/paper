...
object lstm_test {
   ...
   *       - Second    --  Accuracy.
   */
  def one_epoch_at_(task: String)(network: Seq[Layer], data: List[List[DenseVector[Double]]], V: word2vec.Vocabrary): (Double, Double) = {
    ...
    for (d <- data) {
      ...
      val tag = d.tail.map{ i => V.indexOf.filter(_._2 == argmax(i)).par.keys.head }
      val output = out_vec_list.reverse.map{ i => V.indexOf.filter(_._2 == argmax(i)).par.keys.head }

      val str_pair = tag zip output
      num_of_words += str_pair.size
      num_of_correct += str_pair.filter{ case (t, o) => t == o }.size

      E += (d.tail zip out_vec_list.reverse).map{ case (t, o) => -sum(t *:* breeze.numerics.log(o)) }.sum
...
"""
    ...
    val data = io.Source.fromFile(corpus).getLines.map(_.trim.split(" ").take(data_size + test_size)).toArray.flatten
    println("* data loaded")
    V.setVocab(data)
    println("* set vocab")
    ...
    val train_data: List[List[DenseVector[Double]]] = (for (idx <- 0 until data_size - 2 * window) yield {
    ...
    val test_data: List[List[DenseVector[Double]]] = (for (idx <- data_size until data_size + test_size - 2 * window) yield {
      val l = for (i <- idx until idx + window) yield {
        utils.oneHot(V.indexOf(data(i)), V.vocab.size)
      }
      l.toList
    ...
    pll.graph.Line(fig, Seq(list_of_train_E, list_of_test_E, list_of_train_acc, list_of_test_acc).map(v => DenseVector(v.reverse.toArray)), epoch, 2)(
      Seq(
        "train_E", "test_E", "train_acc", "test_acc"))
