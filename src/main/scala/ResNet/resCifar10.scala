...
object resCifar10 {
  ...
  val train_d: Array[Array[Double]] = io.Source
    .fromFile("/home/share/cifar10/train-d.txt")
    .getLines
    .take(ds)
    .map(_.split(",").map(_.toDouble / 256d))
    .toArray
  val r = train_d.map { c =>
  ...
  val tag: Array[Int] = io.Source
    .fromFile("/home/share/cifar10/train-t.txt")
    .getLines
    .map(_.split(",").take(ds).map(_.toInt))
    .toArray
    .flatten

  // read test data.
  val test_d: Array[Array[Double]] = io.Source
    .fromFile("/home/share/cifar10/test-d.txt")
    .getLines
    .take(ds)
    .map(_.split(",").map(_.toDouble / 256d))
    .toArray
  ...
  val t_tag: Array[Int] = io.Source
    .fromFile("/home/share/cifar10/test-t.txt")
    .getLines
    .map(_.split(",").take(ds).map(_.toInt))
    .toArray
    .flatten
