...
object font_GLO {
  def main(args: Array[String]){
    ...
    for(e <- 0 until epoch){
      ...
      if(e == 0 || e % 100 == 0 || e == epoch - 1){
        ...
        write(s"src/main/scala/GLO/results/font_GLO_ds${ds}_epoch${e}of${epoch}_batch${batch}_pad${g.pad}_stride${g.stride}.txt",ys.reverse)
      }
  ...
  def read_labo(fn: String, ds: Int = 100): Array[DenseVector[Double]] = { // {{{
    ...
    val rgb = (for(idx <- 0 until r.size) yield {DenseVector.vertcat(DenseVector.vertcat(r(idx), g(idx), b(idx)))}).toArray
    rgb
  } // }}}
